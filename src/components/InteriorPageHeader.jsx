 ...
 * @param {string} [props.headingEmphasis] Optional heading text that receives the hero accent treatment.
 ...
 */
function InteriorPageHeader({ eyebrow, heading, headingEmphasis, lede }) {
  const emphasisStart = headingEmphasis ? heading.indexOf(headingEmphasis) : -1
  const headingContent = emphasisStart >= 0
    ? (
        <>
          {heading.slice(0, emphasisStart)}
          <Text as="em" className="interior-page-header__emphasis">{headingEmphasis}</Text>
          {heading.slice(emphasisStart + headingEmphasis.length)}
        </>
      )
    : heading
